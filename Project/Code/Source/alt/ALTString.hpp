...
#include <float.h> //JLS for DBL_MIN_10_EXP
#include <stdint.h> //JLS for uint32_t
...
int TVSPrintf(CHAR_TYPE*buffer,const CHAR_TYPE*format,void*ParamList)
{
	...
	const CHAR_TYPE  *sPrefix;		// Prefix string
	...
	Length=0;
	nFlags=0;
	static const CHAR_TYPE strPlus[]={'+',0};
	static const CHAR_TYPE strSub[]={'-',0};
	static const CHAR_TYPE str0x[]={'0','x',0};
	static const CHAR_TYPE str0X[]={'0','X',0};
	static const CHAR_TYPE strNULL[]={0};
	static const CHAR_TYPE strSpace[]={' ',0};
	static const CHAR_TYPE strZero[]={'0',0};
	static const CHAR_TYPE strError[]={'<','E','r','r','o','r','>',0};
...
//		case 'U':
			...
			*str='\0';
#else // JLS
			*myflt((CHAR_TYPE*)&strbuf[0], va_arg(Next, double), Width, Precision, *(format - 1), nFlags | TVSP_SIGN) = '\0';
#endif
			str=strbuf;
			len =  TStrLen(str);
			goto EmitNumber;
...
public:
	...
	virtual void Preallocate(int Length, const CHAR_TYPE* pNewString = NULL)
	{
		...
		const int OldMaxLength = m_MaxLength;
	...
	TString& operator+=(const CHAR_TYPE* pString)
	{
		const int nNewLen = TStrLen(pString);
		Preallocate(nNewLen+m_Length+1,m_pData);
	...
	{
		const int nLen = TVSPrintf((CHAR_TYPE*)NULL, pFormat, va);
		Preallocate(nLen+1);
