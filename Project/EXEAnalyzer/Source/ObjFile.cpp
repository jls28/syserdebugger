...
const char* CObjFile::GetSectionName(int SectionIndex) const
{
	char TmpName[9];
	DWORD StrIndex;
...
const char* CObjFile::GetSymbolNameByIndex(DWORD Index) const
{
	if(Index>m_ObjHead.NumberOfSymbols)
		return NULL;
	return GetSymbolName(&m_ObjSymAry[Index]);
}
const char* CObjFile::GetSymbolName(OBJ_IMAGE_SYMBOL* Sym) const
{
	char* szName;
	if(Sym->N.Name.Short)
	{
...
const DWORD CObjFile::m_ObjMachineSign[] =
{
