...
bool CPEFile::IsRelocArea(ULPOS RVALow,ULPOS RVAHigh) const
{
	ULPOS Low,High;
...
const IMPORT_MODULE* CPEFile::GetImportModule(PCSTR ModuleName) const
{
...
const IMPORT_FUNC* CPEFile::GetImportFunc(const IMPORT_MODULE*pModule,PCSTR FuncName)
{
...
const IMPORT_FUNC* CPEFile::GetImportModuleFunc(PCSTR ModuleName,PCSTR FuncName)
{
	const IMPORT_MODULE*pModule = GetImportModule(ModuleName);
	if(pModule==NULL)
	{
		return NULL;
	}
