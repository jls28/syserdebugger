...
public:
	...
	bool	IsRelocArea(ULPOS RVALow,ULPOS RVAHigh) const;
	...
	const IMPORT_MODULE*GetImportModule(PCSTR ModuleName) const;
	const IMPORT_FUNC*GetImportFunc(const IMPORT_MODULE*pModule,PCSTR FuncName);
	const IMPORT_FUNC*GetImportModuleFunc(PCSTR ModuleName,PCSTR FuncName);
