...
extern const WCHAR * const SymTagName[];
extern const WCHAR* const basetypename[];
extern const WCHAR * const datakindname[];
extern const SDBASETYPENAME BaseTypeNameStructArray[27];
class CTempSymbol
{
	char *m_Name;
	SDTYPE* m_Sdtype;
...
public:
	...
	bool DumpHeaderFile(const char* HeaderFileName);
	//类型项
	...
	const WCHAR *GetSymbolName(IN IDiaSymbol* Symbol,IN DWORD NameMaxLen,OUT WCHAR *Name);
	const WCHAR *GetSymbolTypeName(IN IDiaSymbol* Symbol,IN DWORD NameMaxLen,OUT WCHAR *Name);
	...
	static const char* GetCallConventionName(DWORD dwType);
