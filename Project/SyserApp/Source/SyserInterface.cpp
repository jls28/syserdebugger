...
bool CSyserInterface::InstallServiceEventLog(PCSTR ServiceName,PCSTR MessageDllFullName,DWORD TypesSupported,bool bSystemIoLog)
{
	...
	CHAR zero(0);
	if(RegCreateKeyEx(HKEY_LOCAL_MACHINE,FullName,0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition)!=ERROR_SUCCESS)
		return false;
...
#define SWAP_BYTE_ORDER(x) (((x)<<24) | ((x&0xff00)<<16) | ((x&0xff000)>>8)|(x>>24))
	...
	CHAR zero(0);
	if((lOK = RegCreateKeyEx(HKEY_LOCAL_MACHINE,"SYSTEM\\CurrentControlSet\\Control\\GroupOrderList",0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition))!=ERROR_SUCCESS)
		return false;
...
bool CSyserInterface::InstallFileExtAssociate()
{
	...
	CHAR zero(0);
	if((lOK = RegCreateKeyEx(HKEY_CLASSES_ROOT,".sds",0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition))!=ERROR_SUCCESS)
		return false;
	if(RegSetValueEx(hKey,NULL,0,REG_SZ,(BYTE*)Name,sizeof(Name)) != ERROR_SUCCESS)
		return false;
	RegCloseKey(hKey);
	if(RegCreateKeyEx(HKEY_CLASSES_ROOT,"SdsFile",0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition)!=ERROR_SUCCESS)
		return false;
	...
	if(RegSetValueEx(hKey,"InfoTip",0,REG_SZ,(BYTE*)Name6,sizeof(Name6)) != ERROR_SUCCESS)
		return false;
	RegCloseKey(hKey);
	if(RegCreateKeyEx(HKEY_CLASSES_ROOT,"SdsFile\\shell",0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition)!=ERROR_SUCCESS)
		return false;
	...
	if(RegCreateKeyEx(HKEY_CLASSES_ROOT,"SdsFile\\shell",0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition)!=ERROR_SUCCESS)
		return false;
	RegCloseKey(hKey);
	if(RegCreateKeyEx(HKEY_CLASSES_ROOT,"SdsFile\\shell\\open",0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition)!=ERROR_SUCCESS)
		return false;
	RegCloseKey(hKey);
	if(RegCreateKeyEx(HKEY_CLASSES_ROOT,"SdsFile\\shell\\open\\command",0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition)!=ERROR_SUCCESS)
		return false;
	dwSize = (DWORD)TStrLen(CommandLine)+1;
	if(RegSetValueEx(hKey,NULL,0,REG_SZ,(BYTE*)CommandLine,dwSize) != ERROR_SUCCESS)
		return false;
	RegCloseKey(hKey);
	if(RegCreateKeyEx(HKEY_CLASSES_ROOT,"SdsFile\\shell\\SyserLoad",0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition)!=ERROR_SUCCESS)
		return false;
	if(RegSetValueEx(hKey,NULL,0,REG_SZ,(BYTE*)Name4,sizeof(Name4)) != ERROR_SUCCESS)
		return false;
	RegCloseKey(hKey);
	if(RegCreateKeyEx(HKEY_CLASSES_ROOT,"SdsFile\\shell\\SyserLoad\\command",0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition)!=ERROR_SUCCESS)
		return false;
	dwSize = (DWORD)TStrLen(CommandLine)+1;
	if(RegSetValueEx(hKey,NULL,0,REG_SZ,(BYTE*)CommandLine,dwSize) != ERROR_SUCCESS)
		return false;
	RegCloseKey(hKey);
	if(RegCreateKeyEx(HKEY_CLASSES_ROOT,"exefile\\shell\\ SyserLoad",0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition)!=ERROR_SUCCESS)
		return false;
	if(RegSetValueEx(hKey,NULL,0,REG_SZ,(BYTE*)Name5,sizeof(Name5)) != ERROR_SUCCESS)
		return false;
	RegCloseKey(hKey);
	if(RegCreateKeyEx(HKEY_CLASSES_ROOT,"exefile\\shell\\ SyserLoad\\command",0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition)!=ERROR_SUCCESS)
		return false;
	...
	if(RegSetValueEx(hKey,NULL,0,REG_SZ,(BYTE*)CommandLine,dwSize) != ERROR_SUCCESS)
		return false;
	RegCloseKey(hKey);
	if(RegCreateKeyEx(HKEY_CLASSES_ROOT,"sysfile\\shell",0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition)!=ERROR_SUCCESS)
		return false;
	RegCloseKey(hKey);
	if(RegCreateKeyEx(HKEY_CLASSES_ROOT,"sysfile\\shell\\ SyserLoad",0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition)!=ERROR_SUCCESS)
		return false;
	if(RegSetValueEx(hKey,NULL,0,REG_SZ,(BYTE*)Name5,sizeof(Name5)) != ERROR_SUCCESS)
		return false;
	RegCloseKey(hKey);
	if(RegCreateKeyEx(HKEY_CLASSES_ROOT,"sysfile\\shell\\ SyserLoad\\command",0,&zero,0,KEY_ALL_ACCESS,NULL,&hKey,&Disposition)!=ERROR_SUCCESS)
		return false;
	CommandLine[ModuleFileNameLen]=0;
