...
int CWispDC::DrawSingleLineText(IN const WISP_CHAR* lpString,IN int nCount,INOUT WISP_RECT* lpRect,IN UINT uFormat)
{
	bool isClip = true;
...
int	CWispDC::GetTextHeight(/*const WISP_CHAR Char JLS*/)
{
	return m_pFont->m_Height;
}

int CWispDC::DrawString(IN const WISP_CHAR* lpString,INOUT WISP_RECT* lpRect,IN UINT uFormat,IN UINT nShowLen,IN int nPreLineMaxCharNum)
{
	struct stShowString
	{
		int coordinateX;
		const WISP_CHAR *ShowString;
		int nShowLen;
		int nPixelWidth;
	}ShowString[4];
#define TABEXPENDLEN 9
	...
	int nCharAverageHigh = GetTextHeight(/*WISP_CHAR("A") JLS*/);
	int nTabPixelWidth = nCharAverageWidth;
	int nRightSkipCharNum = 0, nLeftSkipCharNum = 0;
	WISP_RECT rc;
	const WISP_CHAR *ellipsis = WISP_STR("...");
			...
			if(uFormat & WISP_DT_PATH_ELLIPSIS)
			{
				...
				if(FileName != NULL && nPathStrLen > 3)
				{
					...
					if(uFormat & WISP_DT_MODIFYSTRING)
					{
						...
						const_cast<WISP_CHAR *>(PathName)[nPathShowStrLen] = 0x2e;
						const_cast<WISP_CHAR*>(PathName)[nPathShowStrLen+1] = 0x2e;
						const_cast<WISP_CHAR*>(PathName)[nPathShowStrLen+2] = 0x2e;
						for(i = 0; i < nFileNameStrLen;i++)
							const_cast<WISP_CHAR*>(PathName)[nPathShowStrLen+3+i] = FileName[i];
						const_cast<WISP_CHAR*>(FileName)[i] = 0;
					}
				}				
local_2:					
				;
				...
				if(uFormat & WISP_DT_END_ELLIPSIS)
				{
					...
					{									
						for(j = i; j < i + 3 && j < nStringRealLen; j++)
							const_cast<WISP_CHAR*>(lpString)[j] = 0x2e;
					}
...
int CWispDC::CalcTextLineNum(const WISP_CHAR *lpString,int nCount)
{
...
int CWispDC::CalcDrawTextRect(const WISP_CHAR *lpString,WISP_RECT *lpRect,int nStrLen,UINT uFormat, int nTabPixelWidth)
{
...
int CWispDC::GetMaxShowCharNum(const WISP_CHAR *lpString,int nMaxWidth,  int nTabPixelWidth,int nCount)
{
	int sum = 0,i,nTmp;
...
int CWispDC::DrawTabExpendString(int x, int y, const WISP_CHAR *lpString,int nStringRealLen,WISP_RECT rc,int nTabPixelWidth )
{
	WISP_CHAR TabExpendString[TABEXPENDLEN];
	int TabExpendStringPixelWidth;
