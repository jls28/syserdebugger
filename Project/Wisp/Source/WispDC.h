...
public://作裁减
	...
	int		DrawString(IN const WISP_CHAR* lpString,INOUT WISP_RECT* lpRect,IN UINT uFormat,IN UINT nShowLen = 0,IN int nPreLineMaxCharNum=-1);
	...
	int		GetTextHeight(/*const WISP_CHAR Char = 0 JLS*/);
...
private:
	int		DrawSingleLineText(IN const WISP_CHAR* lpString,IN int nCount,INOUT WISP_RECT* lpRect,IN UINT uFormat);
public:
	int		CalcTextLineNum(const WISP_CHAR *lpString,int nCount=-1);
	int		CalcDrawTextRect(const WISP_CHAR *lpString,WISP_RECT *lpRect,int nStrLen,UINT uFormat, int nTabPixelWidth);
	void	AdjustNoClipRect(INOUT WISP_RECT *lpRect,IN UINT uFormat);
	int		GetMaxShowCharNum(const WISP_CHAR *lpString,int nMaxWidth,  int nTabPixelWidth=0,int nCount=-1);
	int		DrawTabExpendString(int x, int y, const WISP_CHAR *lpString,int nStrLen,WISP_RECT rc,int nTabPixelWidth );
public:
